...
import pycparser
from createclone_cross_newtestoldtrain import createast, creategmndata, createseparategraph
import models
...
def test(dataset,epoch):
    # model.eval()
    ff=open("recordFATTnewtestoldtrain"+str(epoch)+".txt",'w')
    count = 0
    correct = 0
    # tp = 0
    # tn = 0
    ...
    for data, label in dataset:
        ...
        if prediction <= args.threshold and label.item() == 1:
            fn += 1
            # print('fn')
    fy=open("recordR8Tnewtestoldtrain"+str(epoch)+".txt",'w')
    print(tp, tn, fp, fn)
    ff.close()
    p = 0.0
    r = 0.0
