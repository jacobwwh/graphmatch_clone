...
def get_sequence_noid(node, sequence):
    token, children = get_token_noid(node), get_child(node)
    if token!='':
        sequence.append(token)
    #print(len(sequence), token)
    for child in children:
        get_sequence_noid(child, sequence)
...
def createast():
    ...
    for rt, dirs, files in os.walk(dirname):
        for file in files:
            ...
            get_sequence_noid(programast,alltokens)
            programfile.close()
            #print(programast)
            #print(alltokens)
